...
- iOS 10.0 and later.
- [Consoles](../03_usage/13_platform/01_consoles.md).
- [Headless Linux and macOS servers](../03_usage/20_export/15_exporting_for_dedicated_servers.md).
...
- GDScript debugger.
   - Support for debugging in threads.
...
- Editor plugins can be downloaded from the internet to
...
- Occlusion culling with rooms and portals.
...
- Maintained c++ and Python bindings provided.
...
- *Images:* [See](../03_usage/21_assets_pipeline/02_importing_images.md).
- *Audio:*
...
- *3D scenes:*
   ...
   - [ESCN](https://github.com/Relintai/pandemonium_engine-blender-exporter) (direct export from Blender).
   - Collada (.dae).
...
- Headless/server binaries can be downloaded for Linux and compiled for macOS.
  Any binary can be used without a window using the `--no-window` command line argument.
...
- Engine and editor written in C++03. In actuality it's somewhere between c++03 and c++11. By default threading uses headers from c++14
  (these are usually available by default on most compilers, as they are just headers).
